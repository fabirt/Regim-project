...
JSON_FILE = "test1_RE-values.json"
MS = "MS"
MI = "MI"
...
mutual_info_data_norm_organized = sorted(mutual_info_data_norm, reverse=True)
order = [None] * length
for i in range(length):
    ref = mutual_info_data_norm_organized[i]
    order[i] = mutual_info_data_norm.index(ref)

mse_data_norm_organized = [0] * length
for i in range(length):
    mse_data_norm_organized[i] = mse_data_norm[order[i]]
